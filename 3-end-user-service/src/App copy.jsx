...
export default class App extends Component {
  ...
  componentDidMount(){
    if (/showSubProfile=true/.test(window.location.hash)) { 
      setTimeout(() => {
       
        this.setState({subProfileOpen: true})
      }, 500);
    }
  ...
  onToggleSubProfileModal = () => this.setState({ subProfileOpen: !this.state.subProfileOpen });
  ...
  render() {
    ...
    const {profileModalOpen,customLoginModalOpen,subProfileOpen} = this.state;
    ...
    return (
      ...
      <div className={cx('app', { 'page-menu-open': this.state.pageMenuOpen, 'profile-menu-open': this.state.profileMenuOpen, config: currentRoute && currentRoute.CONFIG_PAGE})}>
        ...
        <ProfileBar onClick={this.onToggleProfileMenu} showProfile={this.onToggleProfileModal} showCustomLogin={this.onToggleCustomLoginModal} showSubProfile={this.onToggleSubProfileModal}/>  
        ...
        {profileModalOpen && 
          ...
          {subProfileOpen && 
          <Modal className="modalV"show={subProfileOpen} key='customLogin' close={this.onToggleSubProfileModal} > 
              
                <SubProfiles onClose={this.onToggleSubProfileModal}/>
